...
img1 = image.load_img('image/1.jpg', target_size=(64, 64))
...
if(prediction[0][0] > 0.5):
    ...
    plt.text(0, 62, value, color='red', fontsize=10,
             bbox=dict(facecolor='white', alpha=0.8))
    prediction = classifier_dog.predict(img, batch_size=None, steps=1)
    print(prediction[0])
    print(np.argmax(prediction[0]))
    value = dogs[np.argmax(prediction[0])] + \
        ' :%1.10f' % (prediction[0][np.argmax(prediction[0])])
    plt.text(40, 62, value, color='red', fontsize=10,
             bbox=dict(facecolor='white', alpha=0.8))
else:
    ...
    value = cats[np.argmax(prediction[0])] + \
        ' :%1.10f' % (prediction[0][np.argmax(prediction[0])])
    plt.text(40, 62, value, color='red', fontsize=10,
             bbox=dict(facecolor='white', alpha=0.8))
